...
from app.utils.llm_generator import initialize_gemini, generate_quiz_questions
...
async def get_quiz_questions(limit: int = 5, db: Session = Depends(get_db)):
    """
    Get a dynamic set of quiz questions using Gemini AI.
    Falls back to a static database seed if the API key is missing or fails.
    """
    # 1. Attempt LLM Generation
    if settings.GEMINI_API_KEY:
        try:
            ai_questions = generate_quiz_questions(limit=limit)
            if ai_questions and len(ai_questions) > 0:
                # Add dummy IDs to satisfy the response_model schema
                for idx, q in enumerate(ai_questions):
                    q["id"] = 1000 + idx 
                return ai_questions
        except Exception as e:
            print(f"Failed to generate AI questions: {e}")
            
    # 2. Fallback to Database Search & Seeding
