...
(function() {
    ...
    // Auth redirect disabled for Guest/Local mode
...
async function initSidebar() {
    ...
    if (!footer) return;
        ...
        const initials = user ? user.username.substring(0, 2).toUpperCase() : 'GS';
        const username = user ? user.username : 'Guest Analyst';
            ...
            <div class="user-profile-gamified">
                ...
                <div class="user-main">
                    ...
                    <div class="user-details">
                        ...
                        <div class="level-badge">${user ? 'LVL ' + level : 'PUBLIC'}</div>
                    </div>
                </div>
                <div class="xp-container">
                    <div class="xp-header">
                        <span>${user ? 'Progression' : 'Login for XP'}</span>
                ...
                ` : `
                    <button id="btn-portal" class="btn-logout-minimal">
                        <svg style="width: 1.25rem; height: 1.25rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        if (logoutBtn) {
            logoutBtn.addEventListener('click', () => window.api.auth.logout());
        }

        const portalBtn = document.getElementById('btn-portal');
        if (portalBtn) {
            portalBtn.addEventListener('click', () => { window.location.href = 'login.html'; });
        }
}
...
function initAdminDashboard() {
    ...
    async function loadAdminData() {
        try {
            ...
            // Render Trends & Distribution Charts
            renderCharts(stats.trends, stats.distribution);
